...
# Season picker lives in the sidebar and is now WIRED to actually reload data below
# (previously this control existed but MAIN_SEASON was hardcoded to the latest season,
# so switching the selectbox had no visible effect — fixed here).
with st.sidebar:
    st.markdown("### 📅 Season")
...
# ─────────────────────────────────────────────────────────────────────
# Rescale the tilt search grid to what's ACTUALLY observed in the data.
# The previous hardcoded (8°, 62°) range let the optimizer search up to
# 62° even though the real data never goes above ~54° — the model was
# extrapolating ~8° into a region with zero training support, which is
# exactly why some batters showed an "optimal tilt" pinned at 62°.
# ─────────────────────────────────────────────────────────────────────
...
# else: keep the module-level fallback defaults (8.0, 62.0) defined above

# Per-batter local search window: how far (in degrees) from a batter's OWN
# current tilt we allow the optimizer to look. This stops the model from
# recommending a wholesale swing overhaul based on thin extrapolated signal
# far from anything that batter (or similar swings) has actually done.
...
def find_optimal_near(tg, preds, center, window=TILT_SEARCH_WINDOW):
    """Like find_optimal, but restricted to [center-window, center+window].
    Returns (optimal_tilt, optimal_pred, extrapolated) where `extrapolated`
    is True if the best point found sits right at the edge of the searched
    window — a sign the model wants to keep climbing further than we're
    willing to trust it, rather than a genuine interior peak."""
...
# ─────────────────────────────────────────────────────────────────────
# LEAGUE-WIDE OPTIMAL-TILT RANKING TABLE  (ported from baseball_tilt2.py,
# vectorized: one batched model.predict() call instead of one per batter,
# and searched only within TILT_SEARCH_WINDOW of each batter's own current
# tilt so recommendations can't jump to an unsupported global extreme)
# ─────────────────────────────────────────────────────────────────────
...
def build_opt_table(_df, ze, ge, lo, window=TILT_SEARCH_WINDOW):
    ...
    n = len(agg)
    tg = np.linspace(TILT_MIN, TILT_MAX, TILT_GRID_N)          # (T,)
    aa  = agg["avg_aa"].to_numpy()[:, None]                     # (n,1)
    spd = agg["avg_bat_speed"].to_numpy()[:, None]
    ln  = agg["avg_swing_len"].to_numpy()[:, None]
    cur = agg["avg_tilt"].to_numpy()                            # (n,)

    TG  = np.tile(tg, (n, 1))                                   # (n,T)
    ...
    preds = gaussian_filter1d(preds, sigma=1.8, axis=1)          # smooth each batter's curve

    # Restrict the search to each batter's own [current ± window] range.
    win_mask = np.abs(TG - cur[:, None]) <= window
    has_support = win_mask.any(axis=1)
    preds_masked = np.where(win_mask, preds, -np.inf)
    preds_masked[~has_support] = preds[~has_support]           # safety fallback: no window overlap

    idx_opt = np.argmax(preds_masked, axis=1)
    ro_arr  = tg[idx_opt]
    ox_arr  = preds_masked[np.arange(n), idx_opt]

    # Flag rows where the chosen point sits at the edge of the searched
    # window — the model wanted to keep climbing further than we trust it to.
    ...
    # When the model's raw pick is edge-pinned (extrapolated), it has NOT found a
    # genuine interior peak — it just wants to keep climbing past what we trust.
    # In that case, pull the recommendation halfway back toward the batter's own
    # current tilt (a much more conservative ask) and halve the displayed
    # confidence, instead of presenting an aggressive, over-confident change.
    ...
    out = pd.DataFrame({
        ...
        "Confidence": np.round(w_arr_disp, 2),
...
def generate_ai_insight(batter_name, row, lg):
    """Call Claude to produce a scouting paragraph, grounded ONLY in the metrics shown."""
    if row is None: return "No data available for this batter.", None

    api_key = st.secrets.get("ANTHROPIC_API_KEY", os.environ.get("ANTHROPIC_API_KEY", ""))
    if not api_key:
        return ("⚠️ AI Insight is not configured. Add your Anthropic API key as "
                "`ANTHROPIC_API_KEY` in Streamlit secrets (`.streamlit/secrets.toml`) "
                "or as an environment variable to enable this feature."), None
    ...
    n_swings = int(row.get("swings", 0))
    prompt = f"""You are an elite MLB hitting analyst. Analyze the following bat-tracking metrics for {batter_name} ({n_swings} tracked swings, season {MAIN_SEASON}) and produce a concise scouting insight (4-6 sentences). Focus on:
...
- Base every claim ONLY on the metrics above. Do not invent stats, comparisons to other named players, injury history, or biographical facts not given here.
- If you are uncertain about something, say so rather than fabricating it.
- Write in clear, professional scouting language. Do not just repeat the raw numbers — interpret them."""
    ...
    try:
        ...
        source_note = (f"Generated by Claude (Anthropic) from the {n_swings} tracked swings shown above "
                        f"for {batter_name}, season {MAIN_SEASON}. No external data was used.")
        return text, source_note
    except Exception as e:
        return f"AI insight unavailable: {e}", None
...
with st.sidebar:
    ...
    if MODEL_METRICS is not None:
        st.caption(f"Model fit (5-fold CV): **R²={MODEL_METRICS['r2']:.2f} · "
                   f"MAE={MODEL_METRICS['mae']:.3f}**")
    st.markdown("---")
    st.markdown(
        "<div style='font-size:0.72rem;color:#8b949e;line-height:1.5'>"
        "<strong>Data source:</strong> MLB Statcast bat-tracking exports "
        "(<code>players_summary_*.csv</code>, <code>detail_zone_pitchgroup_*.csv</code>), "
        "as provided by the user — not fetched live from MLB/Baseball Savant.<br>"
        "<strong>AI Insight text</strong> (Player Explorer tab) is generated by an LLM "
        "(Claude, Anthropic) from the metrics shown, and should be reviewed by a human "
        "analyst before use — it is a drafting aid, not a verified scouting citation."
        "</div>", unsafe_allow_html=True)
...
# Reuse the encoders already fit on the full dataset (detail_fe) instead of
# re-fitting new LabelEncoders on the filtered slice — refitting here would
# silently produce zone_enc/group_enc values inconsistent with what the
# trained model actually learned.
...
TAB_NAMES=["🏠 Overview","🔍 Player Explorer","⚔️ Comparisons",
           "📈 Trends","🔬 Tilt Optimizer","🏆 Tilt Rankings","📚 Glossary"]
tab_ov,tab_exp,tab_cmp,tab_tr,tab_opt,tab_rank,tab_gl=st.tabs(TAB_NAMES)
...
with tab_exp:
    ...
    if st.button("🤖 Generate AI Insight",key="ai_btn"):
        with st.spinner("Analyzing swing profile …"):
            insight,source_note=generate_ai_insight(sel_exp,row_exp,LG)
...
with tab_opt:
    ...
    if model is None:
        ...
        win6=c4.slider("Search window (°)",5,30,int(TILT_SEARCH_WINDOW),1,key="t6win",
                        help="How far from this batter's current tilt the optimizer is allowed to search.")
        ...
        if p6d.empty:
            ...
            if extrap6:
                st.markdown(f'<div class="warn-box">⚠️ The optimal point sits at the edge of the ±{int(win6)}° '
                             f'search window around {sp6}\'s current tilt — the model wants to keep climbing '
                             f'further, but there isn\'t enough real data near there to trust it. Try widening '
                             f'the window above to see where it settles, or treat this as directional only.</div>',
                             unsafe_allow_html=True)
            # Curve
            fig6=go.Figure()
            if show_ci:
...
# ══════════════════════════════════════════════════════════════════════
# TAB 6 — TILT RANKINGS  (league-wide, ported from baseball_tilt2.py)
# ══════════════════════════════════════════════════════════════════════
with tab_rank:
    ...
    with win_c1:
        st.markdown(
            f'<div class="info-box">Every batter (current filters applied) ranked by gap to model-optimal tilt, '
            f'searched only within ± the window below of their <em>own</em> current tilt (never a wholesale jump '
            f'to an unsupported extreme), then shrunk toward the league optimum <strong>{LEAGUE_OPT_TILT:.1f}°</strong> (K=50).<br>'
            f'🟥 too steep (lower it) &nbsp;·&nbsp; 🟦 too flat (raise it) &nbsp;·&nbsp; 🟩 near-optimal (±2.5°)</div>',
            unsafe_allow_html=True)
    with win_c2:
        rank_window = st.slider("Search window (°)", 5, 30, int(TILT_SEARCH_WINDOW), 1, key="rank_window",
                                 help="How far from a batter's own current tilt the model is allowed to look "
                                      "for an 'optimal' value. Smaller = more conservative, less extrapolation.")
    ...
    if opt_table_dyn.empty:
        ...
        hide_extrap=c4.checkbox("Hide extrapolated",value=False,key="rank_hide_extrap",
            help="Hide batters whose 'optimal' sits at the edge of the search window — "
                 "low-trust recommendations with thin data support.")
        ...
        if n_extrap:
            st.caption(f"⚠️ {n_extrap} of {len(opt_table_dyn)} batters have a recommendation pinned at the edge "
                       f"of the ±{int(rank_window)}° search window — treat those as directional only, "
                       f"not a precise target. Narrow the window above or filter them out with the checkbox.")

        def _delta_style(v):
            if pd.isna(v): return ""
            if v>2.5:  return "background-color:#2d1414;color:#f85149"   # too steep
            if v<-2.5: return "background-color:#0f1f2d;color:#58a6ff"   # too flat
            return "background-color:#0d1f0d;color:#3fb950"              # near-optimal
        ...
        st.download_button("⬇ Download rankings CSV",csv_rank,"tilt_rankings.csv","text/csv")
        ...
        fig_rank=px.bar(_clean(top30,"Δ Tilt","Batter","Current Tilt","Optimal Tilt","Confidence","Swings"),
            x="Δ Tilt",y="Batter",orientation="h",color="Δ Tilt",
            ...
            title="Positive (Δ = Current − Optimal) = too steep, lower the tilt  |  Negative = too flat, raise it")
        ...
        if "Current xwOBA" in tbl.columns and tbl["Current xwOBA"].notna().any():
            ...
            fig_rx=_safe_scatter(sd,x="Δ Tilt",y="Current xwOBA",color="Confidence",size="Swings",
        ...
        if "Gradient Boosting" in model_type and hasattr(model,"feature_importances_"):
            ...
            fig_imp=px.bar(imp.reset_index(),x=0,y="index",orientation="h",
                labels={"0":"Importance","index":"Feature"},color=0,color_continuous_scale="Oranges",
...
# ══════════════════════════════════════════════════════════════════════
# TAB 7 — GLOSSARY & EDUCATION
# ══════════════════════════════════════════════════════════════════════
with tab_gl:
    st.markdown('<div class="section-hdr">Bat Tracking Glossary</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="info-box">The named-player examples below (e.g. "Aaron Judge: ~47°") are '
        'illustrative, rounded approximations used for teaching purposes only — they are '
        '<strong>not</strong> pulled from the CSV data loaded into this dashboard and may not '
        'reflect current-season values. For an exact figure, check the Player Explorer tab, '
        'which reads directly from your dataset.</div>', unsafe_allow_html=True)
