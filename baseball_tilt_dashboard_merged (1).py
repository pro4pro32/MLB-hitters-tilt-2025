...
def train_model():
    ...
    if HAS_PYGAM and len(X) >= 40:
        try:
            gam = LinearGAM(
                s(0, n_splines=12, constraints="none") + s(1, n_splines=10) +
...
def build_opt_table(_df, ze, ge, lo, window=TILT_SEARCH_WINDOW):
    ...
    X = np.column_stack([
        TG.ravel(), AA.ravel(), SPD.ravel(), LN.ravel(),
    ...
    delta = cur - so_arr
    direction = np.where(
        np.abs(delta) <= 2.5, "≈ near-optimal",
        np.where(delta > 0,
                 "↓ flatten " + np.round(np.abs(delta), 1).astype(str) + "°",
...
with st.sidebar:
    st.markdown("### 🔍 Filters")
    spm = st.multiselect(
        "Players (multi-select for comparison)",
        all_real,
        default=all_real[:3] if len(all_real) >= 3 else all_real,
        key="spm"
    )
...
with tab_scat:
    ...
    if x_m in dff.columns and y_m in dff.columns:
        fig = px.scatter(
            dff, x=x_m, y=y_m, color="pitch_group", size="swings",
    ...
    if len(avail_seasons) < 2:
        ...
        if lg_trend:
            ...
            fig_tr.update_layout(
                title=f"{METRIC_META[tr_metric]['label']} — Season Trend (2025–2026)",
                xaxis_title="Season",
...
with tab_opt:
    ...
    st.markdown(
        '<div class="info-box">Partial-dependence curve: all features fixed at batter averages. '
        'Optimal tilt is shrunk toward the league optimum (Bayesian shrinkage, K=50).</div>',
        unsafe_allow_html=True
    )
    ...
    if model is None:
        ...
        if p6d.empty:
            ...
            if n6 < 30:
                st.markdown(
                    '<div class="warn-box">⚠️ Small sample — estimate heavily shrunk toward league optimum.</div>',
                    unsafe_allow_html=True
                )
            if extrap6:
                st.markdown(
                    f'<div class="warn-box">⚠️ Optimal sits at the edge of the ±{int(win6)}° window — treat as directional only.</div>',
                    unsafe_allow_html=True
                )
            ...
            if show_ci:
                ...
                for _ in range(120):
                    _, p = predict_tilt_curve(
                        aa6 + rng.normal(0, 8 * scale),
                        spd6 + rng.normal(0, 3 * scale),
                        len6 + rng.normal(0, 0.3 * scale),
                        ze6, ge6
                    )
                    boots.append(p)
                arr = np.vstack(boots)
            ...
            fig6.add_trace(go.Scatter(
                x=tgl6, y=prl6, mode="lines",
                line=dict(color="#388bfd", width=1.5, dash="dot"), name="League avg features"
            ))
            fig6.add_vrect(
                x0=max(TILT_MIN, ct - win6),
                x1=min(TILT_MAX, ct + win6),
                fillcolor="rgba(63,185,80,0.06)",
                line_width=0,
                annotation_text="trusted search window",
                annotation_position="top left",
                annotation_font_size=9,
                annotation_font_color="#3fb950"
            )
            for xv, col, lbl, dash in [
                (ct, "#3fb950", f"Current {ct:.1f}°", "dash"),
            ...
            X2 = np.column_stack([
                TT.ravel(), AA.ravel(),
                np.full(n2, spd6), np.full(n2, len6),
                np.full(n2, ze6), np.full(n2, ge6),
                TT.ravel() * AA.ravel(), TT.ravel() * ge6
            ])
            Z2 = model.predict(X2).reshape(TT.shape)
            f2d = go.Figure(data=go.Heatmap(
                z=Z2, x=tg2.round(1), y=aa2.round(1),
                colorscale="RdYlGn", colorbar=dict(title="xwOBA"),
                hovertemplate="Tilt: %{x:.1f}°<br>AA: %{y:.1f}°<br>xwOBA: %{z:.3f}<extra></extra>"
            ))
            f2d.add_trace(go.Scatter(
                x=[ct], y=[aa6], mode="markers",
    ...
    if opt_table_dyn.empty:
        ...
        def _delta_style(v):
            if pd.isna(v):
                return ""
            if v > 2.5:
                return "background-color:#2d1414;color:#f85149"
            if v < -2.5:
                return "background-color:#0f1f2d;color:#58a6ff"
        ...
        styler = (tbl.style.map(_delta_style, subset=["Δ Tilt"])
                  if hasattr(tbl.style, "map")
                  else tbl.style.applymap(_delta_style, subset=["Δ Tilt"]))
        st.dataframe(styler.format(fmt), width="stretch", hide_index=True)

        # JEDYNY przycisk download
...
with tab_gl:
    ...
    st.markdown(
        '<div class="info-box">Examples below are illustrative only. For exact figures use the Player Explorer tab.</div>',
        unsafe_allow_html=True
    )
    ...
    if tilt_input >= 45:
        tilt_profile = "🔴 Extreme Uppercut — maximum lift, contact sacrifice"
    elif tilt_input >= 35:
        tilt_profile = "🟠 Power Tilt — solid HR profile"
    elif tilt_input >= 25:
        tilt_profile = "🟢 Balanced — versatile"
    elif tilt_input >= 15:
        tilt_profile = "🔵 Line Drive — contact-first"
    else:
        tilt_profile = "⚪ Flat/Ground Ball — extreme contact"

    if aa_input >= 15:
        aa_profile = "High AA — pop-up risk above 20°"
    elif aa_input >= 6:
        aa_profile = "Optimal AA range"
    elif aa_input >= 0:
        aa_profile = "Neutral — some ground-ball tendency"
    else:
        aa_profile = "Negative AA — ground-ball swing"
