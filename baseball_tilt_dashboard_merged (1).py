...
def generate_ai_insight(batter_name, row, lg):
    ...
    if not api_key:
        return ("⚠️ AI Insight is not configured. Add your Anthropic API key as "
                "`ANTHROPIC_API_KEY` in Streamlit secrets."), None
    metrics_txt = "\n".join(
...
- Base every claim ONLY on the metrics above. Do not invent stats.
- Write in clear, professional scouting language."""
    try:
        resp = requests.post(
        ...
        source_note = (f"Generated by Claude from the {n_swings} tracked swings shown above "
                        f"for {batter_name}, season {MAIN_SEASON}.")
...
# ─────────────────────────────────────────────────────────────────────
# HEATMAP (oryginalna funkcja – bez zmian)
# ─────────────────────────────────────────────────────────────────────
...
<div class="dash-header">
  ...
  <div class="dash-subtitle">BAT TRACKING · SWING PATH TILT · ATTACK ANGLE · 2025–2026</div>
</div>""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    pt_list=["All"]+sorted(_pts["pitch_type"].dropna().unique()) if "pitch_type" in detail_full.columns else ["All"]
...
def apf(df,pg,pt):
    ...
    if pt!="All" and "pitch_type" in df.columns: df=df[df["pitch_type"]==pt]
    return df
...
with tab_exp:
    ...
    if not df_exp.empty:
        ...
        st.download_button(f"⬇ Download {sel_exp} data",csv,f"{sel_exp.replace(' ','_')}_data.csv","text/csv", key="dl_exp")
...
with tab_cmp:
    ...
    if not spm:
        ...
        if rows_dl:
            ...
            st.download_button("⬇ Download comparison data",cmp_csv,"comparison.csv","text/csv", key="dl_cmp")

# ══════════════════════════════════════════════════════════════════════
# TAB 4 — TRENDS
# ══════════════════════════════════════════════════════════════════════
with tab_tr:
    st.markdown('<div class="section-hdr">Season-over-Season Trends (2025–2026)</div>', unsafe_allow_html=True)
...
with tab_opt:
    ...
    st.markdown('<div class="info-box">Partial-dependence curve: all features fixed at batter averages. Optimal tilt is shrunk toward the league optimum (Bayesian shrinkage, K=50).</div>',
                unsafe_allow_html=True)
    ...
    if model is None:
        ...
        if p6d.empty:
            ...
            if extrap6:
                st.markdown(f'<div class="warn-box">⚠️ The optimal point sits at the edge of the ±{int(win6)}° search window.</div>',
                             unsafe_allow_html=True)
...
with tab_rank:
    ...
    if opt_table_dyn.empty:
        ...
        # JEDYNY download_button
        csv_rank=tbl.to_csv(index=False)
        st.download_button("⬇ Download rankings CSV",csv_rank,"tilt_rankings.csv","text/csv", key="dl_rank_unique")
...
with tab_gl:
    ...
    st.markdown(
        '<div class="info-box">The named-player examples below are illustrative approximations used for teaching purposes only.</div>',
        unsafe_allow_html=True)

    glossary=[
        ("⚾ Swing Path Tilt",
         "The angle (in degrees) between the bat's swing path plane and the horizontal ground plane. "
         "Higher tilt = more upward swing. MLB average roughly 25–32°. Elite power hitters often 35–50°.",
         "Aaron Judge: ~47°. Freddie Freeman: ~40°. Luis Arraez: ~15°."),
        ("📐 Attack Angle",
         "The angle at which the bat head meets the ball at contact. Optimal for hard contact is typically 8–15°.",
         "8–14° → sweet spot. < 0° → grounders. > 20° → pop-up risk."),
        ("🚀 Bat Speed",
         "Peak speed of the bat barrel during the swing (mph). MLB average ~70–73 mph. Elite: 77+ mph.",
         "75+ mph → above-average power potential."),
        ("📏 Swing Length",
         "Total distance (ft) the barrel travels from initiation to contact. Optimal range: 6.0–7.5 ft.",
         "6.0–7.0 ft → efficient. > 8.5 ft → long, exploitable."),
        ("📊 xwOBA",
         "Expected Weighted On-Base Average based on launch angle + exit velocity + K/BB. "
         "Removes luck. Scale: < .280 below avg · .310–.340 avg · .360+ above · .400+ elite.",
         "League avg ~.315."),
        ("🎯 Optimal Tilt",
         "The tilt the model predicts would maximize xwOBA for a given batter, with Bayesian shrinkage toward league optimum.",
         "More swings → more player-specific estimate."),
    ]
    ...
    if tilt_input>=45:    tilt_profile="🔴 Extreme Uppercut — maximum lift, contact sacrifice"
    elif tilt_input>=35:  tilt_profile="🟠 Power Tilt — solid HR profile"
    elif tilt_input>=25:  tilt_profile="🟢 Balanced — versatile"
    elif tilt_input>=15:  tilt_profile="🔵 Line Drive — contact-first"
    else:                 tilt_profile="⚪ Flat/Ground Ball — extreme contact"
    if aa_input>=15:      aa_profile="High AA — pop-up risk above 20°"
    elif aa_input>=6:     aa_profile="Optimal AA range"
    elif aa_input>=0:     aa_profile="Neutral — some ground-ball tendency"
    else:                 aa_profile="Negative AA — ground-ball swing"
