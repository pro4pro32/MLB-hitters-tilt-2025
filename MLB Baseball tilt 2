...
def build_opt_table(_df, ze, ge, lo):
    ...
    idx_opt = np.nanargmax(preds, axis=1)
    ro_arr = tg[idx_opt]
    ox_arr = preds[np.arange(n), idx_opt]
    ...
    out = pd.DataFrame({
        ...
        "Current Tilt": agg["avg_tilt"].round(1),
        "Optimal Tilt (raw)": np.round(ro_arr, 1),
        "Optimal Tilt": np.round(so_arr, 1),
        "Δ Tilt": np.round(agg["avg_tilt"].to_numpy() - so_arr, 1),
        "Pred. xwOBA @ Opt.": np.round(ox_arr, 3),
        "Confidence": np.round(w_arr, 2),
...
with tab_rank:
    ...
    st.markdown(
        ...
        f'shrunk toward <strong>{LEAGUE_OPT_TILT:.1f}°</strong> (K=50).<br>'
        f'🟥 too flat &nbsp;·&nbsp; 🟦 too steep &nbsp;·&nbsp; 🟩 near-optimal (±5°)</div>',
        unsafe_allow_html=True)
    ...
    if opt_table.empty:
        ...
        tbl=(opt_table[opt_table["Confidence"]>=min_conf]
             .sort_values(sort_col,ascending=ascending,na_position="last"))

        def _delta_style(v):
            if pd.isna(v): return ""
            if v>5:  return "background-color:#2d1414;color:#f85149"
            if v<-5: return "background-color:#0f1f2d;color:#58a6ff"
        ...
        fig_rank=px.bar(_clean(top30,"Δ Tilt","Batter","Current Tilt","Optimal Tilt","Confidence","Swings"),
            ...
            color_continuous_scale="RdBu",color_continuous_midpoint=0,text="Δ Tilt",
