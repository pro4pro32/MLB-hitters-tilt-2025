...
def load_data():
    # Pliki są teraz w głównym folderze (bez podfolderu)
    players = pd.read_parquet("players_summary_2025.parquet", engine='pyarrow')
    
    detail = pd.read_parquet("detail_zone_pitchgroup_2025.parquet", engine='pyarrow')
...
def make_heatmap(df_p, metric, title):
    ...
    for i in range(3):
        for j in range(3):
            ...
            text = str(zone) if pd.isna(val) else \
                   (f"{zone}\n{int(val)}" if metric == "swings" else \
    ...
    # Labels for L-zones
    offset = border * 0.38
    for pos, z in [((offset, 5 - offset), 11), ((5 - offset, 5 - offset), 12), 
                   ((offset, offset), 13), ((5 - offset, offset), 14)]:
        val = pivot.get(z, np.nan)
        txt = f"{z}\n{val:.3f}" if metric in ["batting_avg","xwoba"] and not pd.isna(val) else \
...
# TAB 5 – Single Batter Analysis (najważniejsza)
with tab5:
